resource "aws_instance" "railsgoat" {
...
provisioner "remote-exec" {    
  ...
  inline = [      
    ...
    "cd vulndocker/railsgoat && sudo docker-compose build && sudo docker-compose run web rails db:setup && docker-compose up -d",
    ]  
  }
}
...
output "GOATED" {
  value = aws_instance.railsgoat.public_ip
}
